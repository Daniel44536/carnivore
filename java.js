...
document.getElementById("Tapabarriga").addEventListener("click", function() {
    updateInfoBox("Tapabarriga");
});
...
function initMap() {
    ...
    // Función para crear un marcador con detalles
    function createMarkerWithDetails(place) {
        ...
        service.getDetails({ placeId: place.place_id }, (details, status) => {
            if (status === google.maps.places.PlacesServiceStatus.OK) {
                ...
                // Crear el contenido del InfoWindow con los detalles
                const infowindow = new google.maps.InfoWindow({
                    content: `
                        <div>
                            <strong>${details.name}</strong><br>
                            ${details.formatted_address || "Dirección no disponible"}<br>
                            ${details.formatted_phone_number || "Teléfono no disponible"}<br>
                            <a href="${details.website || "#"}" target="_blank">Sitio web</a>
                        </div>
                    `,
                });
    ...
    // Buscar carnicerías utilizando `nearbySearch`
    const service = new google.maps.places.PlacesService(map);
    const request = {
        location: santiago, // Ubicación inicial
        radius: 5000, // Radio de búsqueda en metros
        type: ["butcher"], // Tipo de lugar
    };
